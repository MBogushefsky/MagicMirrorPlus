...
WeatherProvider.register("ukmetofficedatahub", {
  ...
  generateWeatherObjectFromCurrentWeather(currentWeatherData) {
    ...
    for (hour in forecastDataHours) {
      ...
      if (
        ...
        currentWeather.windSpeed = this.convertWindSpeed(
          forecastDataHours[hour].windSpeed10m
        );
        currentWeather.windDirection =
          forecastDataHours[hour].windDirectionFrom10m;
        currentWeather.temperature = this.convertTemp(
          forecastDataHours[hour].screenTemperature
        );
        currentWeather.minTemperature = this.convertTemp(
          forecastDataHours[hour].minScreenAirTemp
        );
        currentWeather.maxTemperature = this.convertTemp(
    ...
    let times = this.calcAstroData(
      currentWeatherData.features[0].geometry.coordinates
    );
  ...
  generateWeatherObjectsFromForecast(forecasts) {
    ...
    for (day in forecastDataDays) {
      ...
      if (forecastDate.isSameOrAfter(today)) {
        ...
        forecastWeather.minTemperature = this.convertTemp(
          forecastDataDays[day].nightMinScreenTemperature
        );
        forecastWeather.maxTemperature = this.convertTemp(
          forecastDataDays[day].dayMaxScreenTemperature
        );

        // Using daytime forecast values
        forecastWeather.windSpeed = this.convertWindSpeed(
          forecastDataDays[day].midday10MWindSpeed
        );
        forecastWeather.windDirection =
        ...
        forecastWeather.feelsLikeTemp = this.convertTemp(
  ...
  convertTemp(tempInC) {
    return this.config.tempUnits === "imperial"
      ? (tempInC * 9) / 5 + 32
      : tempInC;
  },
