...
WeatherProvider.register("ukmetoffice", {
  ...
  generateWeatherObjectFromCurrentWeather(currentWeatherData) {
    ...
    for (var i in currentWeatherData.SiteRep.DV.Location.Period) {
      let periodDate = moment.utc(
        currentWeatherData.SiteRep.DV.Location.Period[i].value.substr(0, 10),
        "YYYY-MM-DD"
      );
          ...
          for (var j in currentWeatherData.SiteRep.DV.Location.Period[i].Rep) {
            ...
            if (timeInMins >= p && timeInMins - 180 < p) {
              ...
              currentWeather.humidity =
                currentWeatherData.SiteRep.DV.Location.Period[i].Rep[j].H;
              currentWeather.temperature = this.convertTemp(
                currentWeatherData.SiteRep.DV.Location.Period[i].Rep[j].T
              );
              currentWeather.feelsLikeTemp = this.convertTemp(
                currentWeatherData.SiteRep.DV.Location.Period[i].Rep[j].F
              );
              currentWeather.precipitation = parseInt(
                currentWeatherData.SiteRep.DV.Location.Period[i].Rep[j].Pp
              );
              currentWeather.windSpeed = this.convertWindSpeed(
                currentWeatherData.SiteRep.DV.Location.Period[i].Rep[j].S
              );
              currentWeather.windDirection = this.convertWindDirection(
                currentWeatherData.SiteRep.DV.Location.Period[i].Rep[j].D
              );
              currentWeather.weatherType = this.convertWeatherType(
                currentWeatherData.SiteRep.DV.Location.Period[i].Rep[j].W
              );
            }
          }
        }
  ...
  generateWeatherObjectsFromForecast(forecasts) {
    ...
    for (var j in forecasts.SiteRep.DV.Location.Period) {
      ...
      if (periodDate.isSameOrAfter(moment.utc().startOf("day"))) {
        ...
        weather.minTemperature = this.convertTemp(
          forecasts.SiteRep.DV.Location.Period[j].Rep[1].Nm
        );
        weather.maxTemperature = this.convertTemp(
          forecasts.SiteRep.DV.Location.Period[j].Rep[0].Dm
        );
        weather.weatherType = this.convertWeatherType(
          forecasts.SiteRep.DV.Location.Period[j].Rep[0].W
        );
        weather.precipitation = parseInt(
          forecasts.SiteRep.DV.Location.Period[j].Rep[0].PPd
        );
